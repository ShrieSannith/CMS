...
// Twilio number to use as the caller ID
const fromNumber = '+17743525433'; // Replace with your Twilio number

// Forward Call Endpoint
router.post('/forwardCall', async (req, res) => {
    try {
        console.log('Twilio Request Body:', req.body);
        const phoneNumber = req.body.From; // This is the original caller's number
        ...
        const dial = twimlResponse.dial({
            callerId: fromNumber, // Use your Twilio number as the caller ID
        ...
        dial.number(agent.phoneNumber); // Forward the call to the agent's phone number
    ...
    try {
        ...
        if (notConnectedStatuses.includes(callStatus)) {
            ...
            if (callLog) {
                ...
                await CallLog.findByIdAndUpdate(callLog._id, { status: 'completed', formData: {} });
